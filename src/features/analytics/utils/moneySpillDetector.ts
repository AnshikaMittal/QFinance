  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (duplicateGroup.length > 1) {
      ...
      spills.push({
        id: uuidv4(),
        type: 'duplicate',
        description: `You were charged ${formatAmount(t1.amount)} at ${t1.merchant || t1.description} ${duplicateGroup.length} times in ${cfg.duplicateWindowDays} days — one may be a billing error.`,
        transactions: duplicateGroup.map(t => t.id),
        estimatedWaste: extraCharges.reduce((sum, t) => sum + t.amount, 0),
        period: `${daysBetween(duplicateGroup)} day window`,
        isDismissed: false,
  ...
  for (const [key, group] of merchantGroups) {
    ...
    if (isRecurring) {
      ...
      spills.push({
        ...
        description: `${first.merchant || first.description} charges you ${formatAmount(first.amount)} every ~${Math.round(avgInterval)} days (${group.length} times seen). Canceling saves ${formatAmount(yearlyAmount)}/year.`,
        transactions: group.map(t => t.id),
        estimatedWaste: yearlyAmount,
        period: `${formatAmount(first.amount)} every ~${Math.round(avgInterval)} days`,
        isDismissed: false,
        resolution: 'unresolved',
        detectedAt: new Date(),
      });
  ...
  for (const [categoryId, currentAmount] of Object.entries(currentByCategory)) {
    ...
    if (increase >= cfg.spendingCreepThreshold && currentAmount > 50) {
      ...
      spills.push({
        id: uuidv4(),
        type: 'spending-creep',
        description: `${categoryNames[categoryId] || 'This category'} spending is up ${Math.round(increase * 100)}% — you spent ${formatAmount(currentAmount)} this month vs your usual ${formatAmount(monthlyAvg)}/month.`,
        transactions: txns.map(t => t.id),
        estimatedWaste: currentAmount - monthlyAvg,
        period: `${formatAmount(monthlyAvg)}/mo avg → ${formatAmount(currentAmount)} this month`,
        isDismissed: false,
        resolution: 'unresolved',
        detectedAt: new Date(),
      });
  ...
  return [
    {
      id: uuidv4(),
      type: 'impulse',
      description: `${lateNight.length} purchases made after ${cfg.impulseHourStart > 12 ? cfg.impulseHourStart - 12 : cfg.impulseHourStart} PM this month totaling ${formatAmount(total)} — research shows ~30% of late-night buys are later regretted.`,
      transactions: lateNight.map(t => t.id),
      estimatedWaste: total * 0.3, // estimate 30% of impulse buys are wasteful
      period: `${lateNight.length} late-night purchases`,
      isDismissed: false,
      resolution: 'unresolved',
      detectedAt: new Date(),
    },
